    		...
    		for i in xrange(len(resp["results"])):
				for j in range(len(resp["results"][i]["cars"])):
					...
					if "postal_code" not in resp["results"][i]["address"].keys():
						c.address = resp["results"][i]["address"]["line1"] + ' ' + resp["results"][i]['address']["city"] + ' ' + resp["results"][i]["address"]["country"]
					else:
						c.address = resp["results"][i]["address"]["line1"] + ' ' + resp["results"][i]["address"]["city"] + ' ' + resp["results"][i]["address"]["postal_code"] + ' ' + resp["results"][i]["address"]["country"]
